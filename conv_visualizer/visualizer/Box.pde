class Box {
  ...
  float damping = 0.2;  // Damping coefficient
  float eps = 5.0;  //
  Box trgBox;
  ...
  Box copy() {
    Box copiedBox = new Box(
      this.indices.clone(), // Copy indices array
      this.tensor, // Tensor is reference copy (use tensor.copy() for deep copy if needed)
      this.orgPos.copy(), // PVector is copied through copy()
      this.trgPos.copy(),
      this.orgSize.copy(),
      this.trgSize.copy()
      );
